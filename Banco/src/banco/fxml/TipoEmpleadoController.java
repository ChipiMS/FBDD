...
import database.dao.TipoEmpleadoDAO;
import java.net.URL;
...
public class TipoEmpleadoController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        MySQL db = new MySQL();
        db.Connect();
        dao = new TipoEmpleadoDAO(db.getConnection());
        ...
        table.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                TipoEmpleado g = table.getSelectionModel().getSelectedItem();
                if(g == null){
                ...
                if (agregando) {
                    if (txtClave.getText().trim().length() > 0 && txtDescripcion.getText().trim().length() > 0) {
                        if(dao.isDuplicated(txtClave.getText())){
                            ...
                            msg.setHeaderText("Tipo de Empleado");
                            msg.setContentText("Clave ya existente");
                            msg.show();
                        }
                        else{
                            dao.insert(new TipoEmpleado(txtClave.getText(),txtDescripcion.getText()));
                            Alert msg = new Alert(Alert.AlertType.INFORMATION);
                            msg.setTitle("Guardar");
                            msg.setHeaderText("Tipo de Empleado");
