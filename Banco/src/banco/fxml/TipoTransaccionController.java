...
public class TipoTransaccionController implements Initializable {
    @FXML
    ...
    @FXML
    TableView<TipoTransaccion> table;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        MySQL db = new MySQL();
        db.Connect();
        dao = new TipoTransaccionDAO(db.getConnection());
        ...
        table.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                TipoTransaccion g = table.getSelectionModel().getSelectedItem();
                if(g == null){
                ...
                if (agregando) {
                    if (txtClave.getText().trim().length() > 0 && txtDescripcion.getText().trim().length() > 0) {
                        if(dao.isDuplicated(txtClave.getText())){
                            ...
                            msg.setHeaderText("Tipo de Transaccion");
                            msg.setContentText("Clave ya existente");
                            msg.show();
                        }
                        else{
                            dao.insert(new TipoTransaccion(txtClave.getText(),txtDescripcion.getText()));
                            Alert msg = new Alert(Alert.AlertType.INFORMATION);
                            msg.setTitle("Guardar");
                            msg.setHeaderText("Tipo de Transaccion");
